...
inline Mat getMatchesImage(Mat query, Mat pattern,
		const vector<KeyPoint>& queryKp, const vector<KeyPoint>& trainKp,
		vector<DMatch> matches, int maxMatchesDrawn) {
	Mat outImg;
	...
	drawMatches(query, queryKp, pattern, trainKp, matches, outImg,
			Scalar(0, 200, 0, 255), Scalar::all(-1), vector<char>(),
