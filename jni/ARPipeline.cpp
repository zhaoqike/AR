...
ARPipeline::ARPipeline(const Mat& patternImage,
...
void ARPipeline::init(const Mat& patternImage,
		const CameraCalibration& calibration) {
	m_calibration = calibration;
	m_patternDetector.buildPatternFromImage(patternImage, m_pattern);
	m_patternDetector.train(m_pattern);
...
void ARPipeline::init(const vector<Mat>& patternImageList,
		const CameraCalibration& calibration) {
	m_calibration = calibration;
	for (int i = 0; i < patternImageList.size(); i++) {
		m_patternDetector.buildPatternFromImage(patternImageList[i],
				m_patternList[i]);
	}
...
bool ARPipeline::processFrameFirstStage(Mat& inputFrame) {
	bool patternFound = m_patternDetector.findPatternFirstStage(inputFrame,
			m_patternInfo);
	return patternFound;
}

bool ARPipeline::processFrameSecondStage(Mat& inputFrame, Mat& descriptors) {
	bool patternFound = m_patternDetector.findPatternSecondStage(inputFrame,
			m_patternInfo, descriptors);

	return patternFound;
}

bool ARPipeline::processFrameThirdStage(Mat& inputFrame, Mat& descriptors) {
	bool patternFound = m_patternDetector.findPatternThirdStage(inputFrame,
			m_patternInfo, descriptors);
