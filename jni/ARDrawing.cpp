...
void ARDrawing::drawContours(PatternDetector& pd, Mat& image, PatternTrackingInfo& info)
{
	for (int i = 0; i<pd.m_pattern.keyframeList.size(); i++){
		...
		draw2dContour(pd,image, info, pd.m_pattern.keyframeList[i].points2d, pd.m_lastHomography, CV_RGB(200, 200, 200));
	}
	//perspectiveTransform(m_pattern.keyframeList[m_pattern.keyframeIndex].points2d, info.points2d, m_lastHomography);
	draw2dContour(pd, image, info, pd.m_pattern.keyframeList[pd.m_pattern.keyframeIndex].points2d, pd.m_lastHomography, CV_RGB(200, 200, 200));
...
void ARDrawing::draw2Contours(PatternDetector& pd, Mat& image, PatternTrackingInfo& info, vector<int> matchIndexes, vector<int> estiIndexes)
{
	...
	for (int i = 0; i < matchIndexes.size(); i++)
	{
		//perspectiveTransform(m_pattern.keyframeList[matchIndexes[i]].points2d, info.points2d, m_lastHomography);
		//info.draw2dContour(image, CV_RGB(200, 0, 0),5);
		draw2dContour(pd, image, info, pd.m_pattern.keyframeList[matchIndexes[i]].points2d, pd.m_lastHomography, CV_RGB(200, 0, 0));
	}
	for (int i = 0; i < estiIndexes.size(); i++)
	{
		//perspectiveTransform(m_pattern.keyframeList[estiIndexes[i]].points2d, info.points2d, m_lastHomography);
		//info.draw2dContour(image, CV_RGB(0, 200, 0));
		draw2dContour(pd, image, info, pd.m_pattern.keyframeList[estiIndexes[i]].points2d, pd.m_lastHomography, CV_RGB(0, 200, 0));
	}
...
void ARDrawing::drawTrackedPoints(PatternDetector &pd, Mat &output, Scalar scalar) {
	...
	for (int i = 0; i < pd.after.size(); i++) {
		//conprint<<"i: "<<i<<"  "<<points[0][i].x<<"  "<<points[0][i].y<<endl;
